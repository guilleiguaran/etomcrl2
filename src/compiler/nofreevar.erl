%%
%% replace all Erlang underscore patterns by MCRLFreeN with N an
%% integer
%%
...
free() ->
  {var,0,list_to_atom("MCRLFree"++integer_to_list(put(free,get(free)+1)))}.
...
%
file(Source,Dest) ->
  AbsForms = tl(file(Source)), % removes the -file attribute
  ok = 
...
pattern({var,Line,'_'}) ->
  free(); % We create a new variable: MCRLFreeN, where N is the number
          % of MCRLFree variables that we have already created (stored in
...
gexpr({var,Line,'_'}) -> 
  free();
%% We create a new variable: MCRLFreeN, where N is the number of
%% MCRLFree variables that we have already created (stored in the
%% process dictionary)
