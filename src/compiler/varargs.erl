%% @doc
%% <p> replace all patterns in f(P1,...,PN) -> ...
...
%% added later 
%%
%% replace multiple clauses by one nested 'if' statement</p>

%% But it is not introducing any matches... It is equivalent
%% but in reality it computes the pattern matching, instead of adding
%% the matches to the body of the function

%% The "if" is used for later on have the "+" between the clauses of
%% the Erlang handle_call. Ideally we would like to have mCRL abstract
%% forms which are similar to the Erlang ones but with mCRL semantics.
...
form({function,Line,Name,Arity,Clauses}) -> % We only change something
                                            % in the functions that
                                            % are neither handle_call
                                            % nor handle_cast
...
%%
%% In the case of only one clause:
%%
%% f(P1,P2,...,Pn) -> Body
%%
%% f(Arg1,Arg2,...,Argn) -> 
%%      if () when Assertions -> NewBody
%%
%% In the case of several clauses:
%%
%% f(P1, P2,...,Pn) -> Body1
%% f(Q1, Q2,...,Qn) -> Body2
%%
%% f(Arg1,Arg2,...,Argn) -> 
%%      if () when Assertions1 -> NewBoody1
%%         () when Assertions2 -> NewBoody2
%%
%% New before the name of the variable means that the bindings are
%% already substituted
...
clauses([C0]) ->
    ...
    {clause,Line,[],Guards,Body} = newclause(C0,Head),
    case Guards of
	[] ->
	    [{clause,Line,Head,[],Body}];
	_ ->                            % push 'when' inside
	    [{clause,Line,Head,[],
	      %% Guards here contains the assertions calculated inside
	      %% newclause for the pattern matching between C0 and
	      %% Head 

	      %% Body is the body of the original C0 clause with
	      %% the binding values substituted
	      [{'if',Line,[{clause,Line,[],Guards,Body}]}]}]
    end;
...
%%
%% varargclause( (P1,_,P3,...,Pn) -> B0 )
%%
%% returns:
%% [MCRLArg1, _, MCRLArg2,...,MCRLArgn-1]
%%
...
%% Receives:
%% (H01, H02,..., H0n) -> B0. % Without guards  
%% and the list of new variables for the arguments (Head) 
%%
%% returns:
%% () when Assertions -> B0 with the bindings substituted
%%
%% Assertions is the list of assertions calculated by pattern:patmatch
%% from the pattern between H0 and Head
%% 
%% Bindings is the list of pairs variable and binding value, obtained
%% also from the pattern:patmatch algorithm
...
expr(Expr,Bindings) ->
  case Expr of
       ...
       {cons,Line,H0,T0} ->
         ...
         T1 = expr(T0,Bindings),
         {cons,Line,H1,T1};
       {lc,Line,E0,Qs0} -> % lists comprehension
