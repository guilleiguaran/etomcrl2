...
%% 
%%     $Id: etoe_eval.erl,v 1.2 2004/08/30 13:51:47 juanjosanchez Exp $
%%
%% <p>Modified version by Thomas Arts. November 2000</p> 
%%
...
%% NOTE: From this module the function case_clauses/4 is used in the
%% etomcrl tool so we only explain the effect of using it.

%% PRECOND: The function receives a list of arguments, a list of
%% clauses, a list of bindings and a function.
%% 
%% TODO: Records
%%
%% INVARIANTS:
...
%% The function returns for each of the given clauses a tuple {value,
%% Value, Bindings}, where Value is:

%%     - if in the clause there was a variable it is the value of this
%%     variable

%%     - if in the clause there was a char, integer, float, atom or
%%     string, then Value is the actual value of the char, integer,
%%     float, atom or string.

%%     - if in the clause there was a list or a tuple, then Value is
%%     the list or the tuple with the value of their elements.

%%     - if in the clauses there was a remote call to a function from
%%     the supervisor, the gen_server, the gen_event or the gen_fsm
%%     module, then Value is the module, the function and the value of
%%     the arguments. If the remote function is not in one of those
%%     modules, then the module where it is implemented is analyzed.


%% Bindings are the values that match the variables in the clause.
...
expr({lc,_,E,Qs}, Bs, Lf) -> % lc are considered here
    eval_lc(E, Qs, Bs, Lf);
...
% do not send
	    ...
	    %% It should crash because if this happen this code is not
	    %% analyzable (example: a supervisor who starts a server,
	    %% sends a msg to it and then starts another process. Most
	    %% likely the msg would be lost.
            {value,{gen_server,call,As},Bs3}; 
         _ -> 
	    %% the remote call is to a function in a module which is
	    %% not the supervisor, nor generic server nor the
	    %% gen_event nor the gen_fsm
           case Lf of
                {value,LocalHandler} ->
...
%% @spec case_clauses(Val::tuple(), Clauses::[clause()], Bindings::bindings(), 
%%                    LocalFuncHandler::funHandler()) -> ReturnValue
%% @doc
%% Val: Arguments of the function we are going to evaluate
%% Clauses: Body of the function clauses abstract form 
%% Bindings: list of variable names and their values 
%% LocalFuncHandler: function telling how to evaluate the Erlang program
%% @end
...
%% match_clause (Cs, Val, Bs, Lf) -> {Body, Bindings} or nomatch
%% where 
%% Cs: Body of the function clauses abstract form 
%% Val: Arguments of the function we are going to evaluate
%% Bs: list of variable names and their values 
%% Lf: function telling how to evaluate the Erlang program
%% 
